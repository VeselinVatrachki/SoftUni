...
def setup_players():
    ...
    player_one_sign, player_two_sign = define_sign(player_one_name)
...
while True:
    ...
    try:
        ...
        print("Position must be valid number between 1 and 9 (inclusive) and should be available!")
        continue
    else:
        turns += 1
        ...
        if turns >= 6:
            ...
            first_col = all([el == current_player[1] for el in [board[0][0], board[1][0], board[2][0]]])
            second_col = all([el == current_player[1] for el in [board[0][1], board[1][1], board[2][1]]])
            third_col = all([el == current_player[1] for el in [board[0][2], board[1][2], board[2][2]]])
